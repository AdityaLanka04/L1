...
class TrialSessionRequest(BaseModel):
    fingerprint: str
...
def generate_enhanced_transcript_content(conversation_data, titles):
    ...
    content = (
        ...
        f"**Export Date:** {datetime.now().strftime('%A, %B %d, %Y at %I:%M %p')}\n"
        f"**Total Sessions:** {len(titles)}\n"
        "**Format:** Chronological conversation record\n\n"
        "---\n\n"
        "## Document Information\n\n"
        "This transcript contains complete conversations from your AI chat sessions. "
        "Each session is clearly marked and organized chronologically for easy reference.\n\n"
        "### Navigation Tips\n"
    ...
    for i, session_content in enumerate(messages):
        session_title = titles[i] if i < len(titles) else f"Chat Session {i + 1}"
        content += f"## Session {i + 1}: {session_title}\n\n"
        content += "**Session Overview:** Detailed conversation transcript\n\n"
    ...
    # Build summary separately to avoid f-string nesting issues
    total_exchanges = sum(
        len(re.findall(r"Q: (.*?)\nA: (.*?)(?=\nQ:|$)", session, re.DOTALL))
...
async def start_trial_session(
    request: Request,
    trial_data: TrialSessionRequest,
    db: Session = Depends(get_db)
):
    """Start a new trial session with fingerprint and IP tracking"""
    try:
        # Get client IP address
        client_ip = get_client_ip(request)
        
        # Hash the fingerprint for privacy
        fingerprint_hash = hashlib.sha256(trial_data.fingerprint.encode()).hexdigest()
        
        # Check if this fingerprint or IP is already blocked
        existing_session = db.query(TrialSession).filter(
            and_(
                TrialSession.fingerprint_hash == fingerprint_hash,
                TrialSession.created_at > datetime.utcnow() - timedelta(hours=24)
            )
        ).first()
        
        if existing_session:
            existing_session.is_blocked = True
            existing_session.block_reason = "repeated_access"
            db.commit()
            return {"status": "blocked", "reason": "repeated_access"}
        
        # Check IP-based blocking (same IP within 24 hours)
        if client_ip and client_ip != "unknown":
            ip_session = db.query(TrialSession).filter(
                and_(
                    TrialSession.ip_address == client_ip,
                    TrialSession.created_at > datetime.utcnow() - timedelta(hours=24)
                )
            ).first()
            
            if ip_session:
                # Create new session but mark as blocked
                new_session = TrialSession(
                    fingerprint_hash=fingerprint_hash,
                    ip_address=client_ip,
                    user_agent=trial_data.userAgent,
                    is_blocked=True,
                    block_reason="ip_reuse",
                    expires_at=datetime.utcnow() + timedelta(hours=24)
                )
                db.add(new_session)
                db.commit()
                return {"status": "blocked", "reason": "ip_reuse"}
        ...
        print(f"Error starting trial session: {str(e)}")
        return {"status": "error", "message": "Failed to start trial session"}

@app.post("/check_trial")
async def check_trial_status(
    request: Request,
    trial_check: TrialCheckRequest,
    db: Session = Depends(get_db)
):
    """Check if a fingerprint/IP combination is blocked"""
    try:
        client_ip = get_client_ip(request)
        fingerprint_hash = hashlib.sha256(trial_check.fingerprint.encode()).hexdigest()
        
        # Check fingerprint-based blocking
        fingerprint_session = db.query(TrialSession).filter(
            and_(
                TrialSession.fingerprint_hash == fingerprint_hash,
                TrialSession.is_blocked == True,
                TrialSession.created_at > datetime.utcnow() - timedelta(hours=24)
            )
        ).first()
        ...
        if fingerprint_session:
            return {
                "blocked": True,
                "reason": fingerprint_session.block_reason,
                "message": "Trial access restricted"
            }
        ...
        if client_ip and client_ip != "unknown":
            ...
            if ip_session:
                return {
                    ...
                    "message": "Trial already used from this location"
                }
        
        return {
            "blocked": False,
            "message": "Trial access allowed"
        }
        
    except Exception as e:
        print(f"Error checking trial status: {str(e)}")
        return {"blocked": False, "message": "Check failed, allowing access"}

# Helper function to get client IP (add this to your main.py)
...
# Cleanup endpoint for expired trial sessions (add this as well)
@app.post("/cleanup_expired_trials")
async def cleanup_expired_trials(db: Session = Depends(get_db)):
    """Clean up expired trial sessions (run this periodically)"""
    try:
        # Delete sessions older than 24 hours
        expired_sessions = db.query(TrialSession).filter(
            TrialSession.created_at < datetime.utcnow() - timedelta(hours=24)
        ).delete()
        ...
        return {
            ...
            "message": f"Cleaned up {expired_sessions} expired trial sessions"
        }
        
    except Exception as e:
        print(f"Error cleaning up trials: {str(e)}")
        return {"status": "error", "message": "Cleanup failed"}

# Optional: Get trial statistics (admin endpoint)
@app.get("/trial_statistics")
async def get_trial_statistics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Today's trials
        today_trials = db.query(TrialSession).filter(
            TrialSession.created_at >= datetime.combine(today, datetime.min.time())
        ).count()
        
        # Weekly trials
        weekly_trials = db.query(TrialSession).filter(
            TrialSession.created_at >= week_ago
        ).count()
        
        # Blocked sessions
        blocked_today = db.query(TrialSession).filter(
            and_(
                TrialSession.created_at >= datetime.combine(today, datetime.min.time()),
                TrialSession.is_blocked == True
            )
        ...
        # Total unique IPs
        unique_ips = db.query(TrialSession.ip_address).distinct().count()
        ...
        return {
            ...
            "block_rate": round((blocked_today / today_trials * 100), 2) if today_trials > 0 else 0
        }
        
    except Exception as e:
        print(f"Error getting trial statistics: {str(e)}")
