...
import { 
  ...
  Maximize2, Minimize2, Link as LinkIcon, TrendingDown, ArrowLeft
} from 'lucide-react';
...
const StudyRecommendationsPanel = ({ recommendations, onClose }) => {
  return (
    <div className="cw-recommendations-panel">
      <div className="cw-recommendations-header">
        <h3>
          <Lightbulb size={20} />
      ...
      <div className="cw-recommendations-content">
        {recommendations.map((rec, index) => (
          <div key={index} className="cw-recommendation-card">
            <div className="cw-recommendation-priority" data-priority={rec.priority}>
              {rec.priority === 'high' && <AlertCircle size={16} />}
              {rec.priority === 'medium' && <Info size={16} />}
              {rec.priority === 'low' && <CheckCircle size={16} />}
            </div>
            <div className="cw-recommendation-content">
              <h4>{rec.title}</h4>
              <p>{rec.description}</p>
              <div className="cw-recommendation-meta">
                <span><Clock size={12} /> {rec.estimated_time}</span>
                <span><Target size={12} /> {rec.impact}</span>
              </div>
            </div>
            <button className="cw-recommendation-action">
              <ArrowRight size={16} />
            </button>
          </div>
        ))}
...
// ==================== CONCEPT COMPARISON COMPONENT ====================

const ConceptComparisonView = ({ concept1, concept2 }) => {
  return (
    <div className="cw-comparison-view">
      <div className="cw-comparison-header">
        <h3>Concept Comparison</h3>
      </div>
      <div className="cw-comparison-grid">
        <div className="cw-comparison-column">
          <h4>{concept1.concept_name}</h4>
          <div className="cw-comparison-stats">
            <div className="cw-comparison-stat">
              <span>Mastery</span>
              <span>{Math.round(concept1.mastery_level * 100)}%</span>
            </div>
            <div className="cw-comparison-stat">
              <span>Content</span>
              <span>{concept1.notes_count + concept1.quizzes_count + concept1.flashcards_count}</span>
            </div>
          </div>
        </div>
        <div className="cw-comparison-divider">
          <ArrowRight size={24} />
        </div>
        <div className="cw-comparison-column">
          <h4>{concept2.concept_name}</h4>
          <div className="cw-comparison-stats">
            <div className="cw-comparison-stat">
              <span>Mastery</span>
              <span>{Math.round(concept2.mastery_level * 100)}%</span>
            </div>
            <div className="cw-comparison-stat">
              <span>Content</span>
              <span>{concept2.notes_count + concept2.quizzes_count + concept2.flashcards_count}</span>
            </div>
          </div>
        </div>
      </div>
...
const ProgressTimeline = ({ concepts, timeRange }) => {
  return (
    <div className="cw-timeline-view">
      ...
      <div className="cw-timeline-content">
        {concepts.map((concept, index) => (
          <div key={index} className="cw-timeline-item">
            <div className="cw-timeline-marker">
              <div className="cw-timeline-dot" style={{ backgroundColor: getCategoryColor(concept.category) }}></div>
              {index < concepts.length - 1 && <div className="cw-timeline-line"></div>}
            </div>
            <div className="cw-timeline-card">
              ...
              <div className="cw-timeline-progress">
                <div className="cw-timeline-progress-bar">
                  <div 
                    className="cw-timeline-progress-fill"
                    style={{ width: `${concept.mastery_level * 100}%` }}
                  ></div>
                </div>
                <span>{Math.round(concept.mastery_level * 100)}%</span>
              </div>
            </div>
          </div>
        ))}
...
const exportToMarkdown = (concepts, connections) => {
  let markdown = '# Concept Web Export\n\n';
  markdown += `Generated on: ${new Date().toLocaleString()}\n\n`;
  markdown += `## Concepts (${concepts.length})\n\n`;
  
  concepts.forEach(concept => {
    markdown += `### ${concept.concept_name}\n`;
    markdown += `- **Category**: ${concept.category}\n`;
    markdown += `- **Mastery**: ${Math.round(concept.mastery_level * 100)}%\n`;
    if (concept.description) {
      markdown += `- **Description**: ${concept.description}\n`;
    }
    markdown += `- **Content**: ${concept.notes_count} notes, ${concept.quizzes_count} quizzes, ${concept.flashcards_count} flashcards\n\n`;
  });
  ...
  connections.forEach(conn => {
    ...
    const target = concepts.find(c => c.id === conn.target_id);
    if (source && target) {
      markdown += `- ${source.concept_name} → ${target.concept_name} (${conn.connection_type})\n`;
    }
  });
...
const exportToCSV = (concepts) => {
  let csv = 'Name,Category,Mastery,Notes,Quizzes,Flashcards,Connections\n';
  ...
  concepts.forEach(concept => {
    ...
    csv += `"${concept.concept_name}","${concept.category}",${Math.round(concept.mastery_level * 100)},${concept.notes_count},${concept.quizzes_count},${concept.flashcards_count},${connCount}\n`;
  });
