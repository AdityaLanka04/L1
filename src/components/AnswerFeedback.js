...
export default function AnswerFeedback({ resourceType, resourceId, sources = [], className = '' }) {
  ...
  return <div className={`learning-flow flow-feedback-tools ${className}`.trim()}>
    {sources.length > 0 && <details><summary>Source references ({sources.length})</summary>{sources.map((source, i) => <p key={i}><strong>{source.book_title || source.filename || source.source_label || 'Course material'}{source.page ? ` · page ${source.page}` : ''}</strong>{source.snippet && <><br />{source.snippet}</>}</p>)}</details>}
